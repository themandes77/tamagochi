...
class JsonStorageReadResult {
  const JsonStorageReadResult({required this.status, this.payload});
...
class JsonFileStorage {
  ...
  Future<JsonStorageReadResult> _readInternal() async {
    final paths = await _paths();
    final current = await _inspect(paths.current, JsonRecoverySource.current);
    ...
    final backup = await _inspect(paths.backup, JsonRecoverySource.backup);
    ...
    _report(code: 'recovery_failed', details: const <String, Object?>{});
    return const JsonStorageReadResult(
      status: JsonStorageReadStatus.resetRequired,
    );
  }
  ...
  Future<void> _writeInternal(JsonPayload payload) async {
    ...
    await _deleteIfExists(paths.temporary);
    await paths.temporary.writeAsString(jsonEncode(document), flush: true);
  ...
  Future<_InspectedDocument> _inspect(
    ...
    try {
      ...
      payload = requireStringObjectMap(root['payload'], description: 'payload');
  ...
  void _report({required String code, required Map<String, Object?> details}) {
