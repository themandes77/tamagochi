...
class CrossModuleTransactionCoordinator {
  ...
  final Map<String, List<_PendingWriteAheadTarget>> _pendingWriteAheadTargets =
  ...
  Future<void> _commitWriteAheadInternal({
    ...
    try {
      ...
      for (final entry in targetPayloads.entries) {
        ...
        final durableBeforeChecksum =
            previousPending?.targetChecksum ??
  ...
  void _scheduleSuccessfulCleanup({
    ...
    unawaited(
      _cleanupSuccessfulTransaction(transactionId: transactionId, type: type),
    );
  }
