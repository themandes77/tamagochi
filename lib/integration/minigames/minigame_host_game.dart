...
class MinigameHostGame extends FlameGame {
  ...
  final Future<bool> Function(GameCostPolicy costPolicy) onGameStartRequested;
  final VoidCallback onGameStartRejected;
  final Future<void> Function(MinigameSessionResult result) onGameOverDetected;
  ...
  void update(double dt) {
    ...
    if (isGameOver != null && isGameOver() && !_gameOverCheckpointStarted) {
  ...
  Future<void> _checkpointGameOver() async {
    ...
    try {
      await onGameOverDetected(
        MinigameSessionResult(gameId: costPolicy.gameId, score: readScore()),
      );
    } catch (error, stackTrace) {
      FlutterError.reportError(
        FlutterErrorDetails(
