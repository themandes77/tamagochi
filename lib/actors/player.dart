...
class Nti extends PositionComponent with TapCallbacks {
  ...
  FutureOr<void> onLoad() async {
    ...
    _speechBubble = NtiSpeechBubble(position: Vector2((size.x - 260) / 2, -96));
  ...
  void update(double dt) {
    ...
    final actionSuppression = math.max(_cleaningVisual, _eatingActionVisual);
    ...
    final baseScale = _viewportScale * (1 + breath + _reactionVisual * 0.06);
    scale = Vector2(
    ...
    final idleAngleAmplitude =
        0.007 * (1 - (energy * 0.42 + fun * 0.18).clamp(0.0, 0.62).toDouble());
    angle =
...
class NtiFace extends PositionComponent {
  ...
  void _drawMouth(
    ...
    if (cleanliness > 0.22) {
      _drawUneasyMouth(canvas, center: center, intensity: cleanliness * 0.55);
      return;
    }
    ...
    if (dirt > 0.035) {
      ...
      for (var index = 0; index < 2; index++) {
        ...
        canvas.drawCircle(
          ...
          size.x * 0.009,
          Paint()
            ..color = const Color(0xFF624936).withValues(alpha: dirt * 0.32),
        );
      }
    }
