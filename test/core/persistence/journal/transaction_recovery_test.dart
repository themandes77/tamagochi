...
void main() {
  ...
  test(
    'journal schema 1 migrates through schema 3 without inventing beforePayload',
    () {
  ...
  test(
    'removes every legacy conflict with zero applied participants',
    () async {
        ...
        repository.items['feed_$index'] = JournalTransaction(
          ...
          participants: <String, JournalParticipantRecord>{
            'pet': JournalParticipantRecord(
              participantKey: 'pet',
              beforeChecksum: checksum.checksumCanonical(<String, Object?>{
                'value': 1,
              }),
              targetChecksum: checksum.checksumCanonical(<String, Object?>{
                'value': 2,
              }),
              targetPayload: <String, Object?>{'value': 2},
  ...
  test(
    'rolls back applied participant and preserves external non-applied state',
    () async {
      final repository = _MemoryJournalRepository();
  ...
  test(
    'legacy partial application without beforePayload cannot be guessed',
    () async {
      final repository = _MemoryJournalRepository();
      final now = DateTime.utc(2026, 8, 10, 18, 30);
      final before = <String, Object?>{'value': 1};
  ...
  test(
    'a pending transaction that discovers conflict is resolved in same recovery',
    () async {
      final repository = _MemoryJournalRepository();
      final now = DateTime.utc(2026, 8, 10, 18, 30);
      final before = <String, Object?>{'value': 1};
  ...
  test(
    'fast path uses durable baseline without redundant participant writes',
    () async {
      final repository = _MemoryJournalRepository();
  ...
  test(
    'pending partial transaction infers target and resumes without status checkpoints',
    () async {
      final repository = _MemoryJournalRepository();
      final now = DateTime.utc(2026, 8, 11, 10);
      final petBefore = <String, Object?>{'value': 1};
  ...
  test(
    'schema 3 recovery accepts durable before distinct from logical before',
    () async {
      final repository = _MemoryJournalRepository();
  ...
  test(
    'write-ahead commits rapid actions while materialization stays serial',
    () async {
      final repository = _MemoryJournalRepository();
      final now = DateTime.utc(2026, 8, 12, 10, 30);
      final gate = Completer<void>();
      final pet = _DeferredMemoryParticipant('pet', <String, Object?>{
        'value': 1,
      }, persistGate: gate.future);
      final store = _DeferredMemoryParticipant('store', <String, Object?>{
        'value': 10,
      }, persistGate: gate.future);
  ...
  test('journal corruption stays fail-closed across retries', () async {
    ...
    await File(
      '${directory.path}${Platform.pathSeparator}transaction_journal.json',
...
class _DeferredMemoryParticipant implements WriteAheadTransactionParticipant {
